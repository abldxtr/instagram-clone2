...
export const newProductSchema = z.object({
    ...
    image: typeof window === "undefined" ? z.any() : z.instanceof(FileList).transform((fileList) => fileList[0]).refine((file) => !file || file.size !== 0 || file.size <= MAX_FILE_SIZE, `Max image size is ${MAX_FILE_SIZE}MB`).refine((file) => !file || file.type === "" || ACCEPTED_IMAGE_TYPES.includes(file.type), "Only .jpg, .jpeg, and .png formats are supported"),
    // image: z
    //     .any()
...
function CreatePage() {
    ...
    const onFileChange = (event) => {
        ...
        form.setValue("image", file ? { size: file.size, type: file.type, name: file.name } : undefined);
